...
public class RedisUtil {
     ...
     * @param expire 过期时间（毫秒计）
     */
    public void hset(String key,String hashkey,Object domain,Integer expire){
    	@SuppressWarnings({ "unchecked", "rawtypes" })
		Jackson2JsonRedisSerializer jackson2JsonRedisSerializer = new Jackson2JsonRedisSerializer(Object.class);
    	StringRedisSerializer  string=new StringRedisSerializer ();
		if("model".equals(hashkey)) {
	...
	public Object getModel(String token) {    	
		  ...
		  //System.out.println(JSONObject.toJSONString(redisTemplate.opsForHash().get(token, "model"))); 
		  //System.out.println(json); 
		  //Object objs=JSONObject.parseObject(json, Object.class);
    	String json=redisTemplate.opsForHash().get(token, "model").toString();
    	JSONObject obj=JSONObject.parseObject(json);
    	UserEntity user=JSONObject.parseObject(json, UserEntity.class);
    	user.setAuthorities(new HashSet<GrantedAuthority>((Collection<? extends GrantedAuthority>) obj.get("authorities")));
        return user;
    }
    /**
     * 查询token下的刷新时间
    ...
    public void setTokenRefresh(String token,UserEntity user,String ip){
        //刷新时间
        ...
        hset(token, "username",user.getUsername(),expire);
        hset(token, "model",user,expire);
